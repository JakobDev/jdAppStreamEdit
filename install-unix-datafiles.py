...
def write_translation_status(project_root: str, appstream_path: str) -> None:
    ...
    translation_dir = os.path.join(project_root, "jdAppStreamEdit", "translations")
    for translation_file in os.listdir(translation_dir):
        if not translation_file.endswith(".ts"):
            continue

        language = translation_file.removeprefix("jdAppStreamEdit_").removesuffix(".ts")
...
def main() -> None:
    ...
    appstream_path = os.path.join(args.prefix, "share", "metainfo", "page.codeberg.JakobDev.jdAppStreamEdit.metainfo.xml")
    ...
    subprocess.run(["msgfmt", "--xml", "--template", os.path.join(project_root, "deploy", "page.codeberg.JakobDev.jdAppStreamEdit.metainfo.xml"), "-d", os.path.join(project_root, "deploy", "translations"), "-o", appstream_path], check=True)
